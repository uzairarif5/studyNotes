...
class SubListInner extends React.Component<SubListInnerPropsType> {
	static globalNum = 0;
	curNum: number;
	prevImage: string;
	parentDir: string;

	constructor(props: SubListInnerPropsType) {
		super(props);
	...
	imgFunc(ci: string, el: any){
		let selectedEl = $(el).next();
		switch(ci){
			case "right":
				el.src = iconDown;
				selectedEl.slideDown("fast",
					()=>document.getElementById("root")!.scrollBy(0, 1) //to make down button visible again
				);
				break;
			case "left":
				el.src = iconDown;
...
function SubList(props: any){
	const closingState = useSelector((state: StateType) => state.currentlyClosingLists); 
