...
function viewImgHandler(input, type){
  let renderedEl;
  switch(type){
    case "line":
...
export function drawLine(input, forImg = false){
  let xAxis =
    input.data[0]["xVal"] ?
    <XAxis
      dataKey="xVal"
  ...
  return <li className='chartContainer' onClick={forImg ? null : ()=>{viewImgHandler(input, "line")}}>
...
export function drawBar(input, forImg = false){
  let data = input.data;
  let h = (input.h === undefined)? 250 : input.h;
  let marginLeft = (input.yLabel !== "") ? 0 : -20;
  let onClickFunc = forImg ? null : ()=>{viewImgHandler(input, "bar")};
...
export function drawHis(input, forImg = false){
  let data = input.data;
  let h = (input.h === undefined) ? 250 : input.h;
  let marginLeft = (input.yLabel !== "") ? 0 : -20;
  let onClickFunc = forImg ? null : ()=>{viewImgHandler(input, "his")};
